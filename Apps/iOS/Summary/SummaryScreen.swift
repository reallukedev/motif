import SwiftUI
import MotifCore

struct SummaryScreen: View {
    @Environment(AppModel.self) private var model
    @Environment(\.horizontalSizeClass) private var sizeClass
    @AppStorage("statsRange") private var range: StatsRange = .month
    @State private var summary: StatsSummary?
    @State private var showsSettings = LaunchScene.opensSettings

    var body: some View {
        ScrollViewReader { proxy in
            ScrollView {
                VStack(alignment: .leading, spacing: Metrics.sectionSpacing) {
                    if model.hasBanner || model.capture?.nowPlaying != nil {
                        VStack(spacing: 12) {
                            StoreWarningBanner()
                            SampleDataBanner()
                            NowPlayingCard()
                        }
                    }

                    if model.library.isLoaded, model.library.history.isEmpty {
                        WelcomeView()
                            .padding(.top, 40)
                    } else {
                        RangePicker(range: $range)
                        if let summary {
                            if summary.captureCount == 0 {
                                ContentUnavailableView(
                                    "Nothing Played \(Text(range.phrase))",
                                    systemImage: "waveform",
                                    description: Text("Try a longer range.")
                                )
                            } else {
                                SummaryContent(summary: summary, isWide: sizeClass == .regular)
                            }
                        }
                    }
                }
                .padding(.horizontal)
                .padding(.bottom, 24)
                .frame(maxWidth: 960)
                .frame(maxWidth: .infinity)
            }
            .animation(.smooth, value: model.capture?.nowPlaying)
            .onChange(of: summary != nil) {
                if let anchor = LaunchScene.scrollAnchor {
                    proxy.scrollTo(anchor, anchor: .top)
                }
            }
        }
        .background(GroupedBackground())
        .navigationTitle("Summary")
        .toolbar {
            ToolbarItem(placement: .topBarTrailing) {
                Button("Settings", systemImage: "gearshape") { showsSettings = true }
            }
        }
        .sheet(isPresented: $showsSettings) {
            SettingsSheet()
        }
        .refreshable {
            if !model.isShowingSampleData { await model.capture?.catchUp() }
        }
        .task(id: "\(range.rawValue)|\(model.library.revision)|\(sizeClass == .regular)") {
            summary = StatsCalculator.summary(
                range: range,
                history: model.library.history,
                sessions: model.library.sessions,
                topLimit: sizeClass == .regular ? 8 : 5
            )
        }
    }
}

/// Everything below the range picker. `isWide` puts some cards side by side on iPad.
struct SummaryContent: View {
    let summary: StatsSummary
    var isWide = false

    var body: some View {
        VStack(alignment: .leading, spacing: Metrics.sectionSpacing) {
            VStack(spacing: 12) {
                ListeningCard(summary: summary)
                GlanceGrid(summary: summary, columns: isWide ? 4 : 2)
            }

            if !summary.insights.isEmpty {
                VStack(alignment: .leading, spacing: 10) {
                    SectionHeader(title: "Highlights") {
                        if summary.insights.count > 3 {
                            NavigationLink("Show All", value: Route.highlights(summary.range))
                        }
                    }
                    EqualHeightRows(items: Array(summary.insights.prefix(isWide ? 4 : 3)), columns: isWide ? 2 : 1) { insight in
                        HighlightCard(insight: insight)
                    }
                }
                .id("highlights")
            }

            if !summary.topArtists.isEmpty {
                VStack(alignment: .leading, spacing: 10) {
                    SectionHeader(title: "Top Artists") {
                        NavigationLink("See All", value: Route.chart(.artists))
                    }
                    TopArtistsShelf(artists: summary.topArtists)
                }
                .id("artists")
            }

            if !summary.topSongs.isEmpty {
                VStack(alignment: .leading, spacing: 10) {
                    SectionHeader(title: "Top Songs") {
                        NavigationLink("See All", value: Route.chart(.songs))
                    }
                    TopSongsCard(songs: summary.topSongs)
                }
                .id("songs")
            }

            if !summary.topAlbums.isEmpty {
                VStack(alignment: .leading, spacing: 10) {
                    SectionHeader(title: "Top Albums") {
                        NavigationLink("See All", value: Route.chart(.albums))
                    }
                    TopAlbumsShelf(albums: summary.topAlbums)
                }
                .id("albums")
            }

            VStack(alignment: .leading, spacing: 10) {
                SectionHeader("When You Listen")
                LazyVGrid(columns: columns, spacing: 12) {
                    Card { ListeningClockCard(hourly: summary.hourly) }
                    Card {
                        VStack(alignment: .leading, spacing: 12) {
                            CardLabel(title: "By Day", systemImage: "calendar")
                            WeekdayChart(weekdays: summary.weekdays)
                            if let busiest = summary.weekdays.max(by: { $0.count < $1.count }), busiest.count > 0 {
                                Text("Busiest on \(Calendar.current.weekdaySymbols[busiest.weekday - 1])s.")
                                    .font(.caption)
                                    .foregroundStyle(.secondary)
                            }
                        }
                    }
                }
            }
            .id("rhythm")

            if summary.sourcesAreInformative || summary.sources.contains(where: { $0.kind == .radio }) {
                LazyVGrid(columns: columns, spacing: 12) {
                    if summary.sourcesAreInformative {
                        Card {
                            VStack(alignment: .leading, spacing: 14) {
                                CardLabel(title: "How You Listened", systemImage: "headphones")
                                SourcesDonut(sources: summary.sources)
                            }
                        }
                    }
                    if summary.sources.contains(where: { $0.kind == .radio }) {
                        RadioCard(summary: summary)
                    }
                }
            }

            Text("Listening time is estimated from when each song started.")
                .font(.footnote)
                .foregroundStyle(.secondary)
                .frame(maxWidth: .infinity)
        }
    }

    private var columns: [GridItem] {
        Array(repeating: GridItem(.flexible(), spacing: 12, alignment: .top), count: isWide ? 2 : 1)
    }
}
