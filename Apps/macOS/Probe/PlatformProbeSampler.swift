...
final class PlatformProbeSampler {
    ...
    private var spotifyObserver: SpotifyProbeObserver?
    ...
    func environmentRows() -> [(key: String, value: String)] {
        [
            ...
            ("Spotify running", String(MusicScripting.isSpotifyRunning)),
    ...
    func start(log: ProbeLog) {
        ...
        let spotify = SpotifyProbeObserver(log: log)
        spotify.start()
        spotifyObserver = spotify
...
/// Logs Spotify's public playback notification, to check it still fires and what it
/// carries. Spotify plays are never captured.
@MainActor
final class SpotifyProbeObserver {
    ...
    func start() {
        token = DistributedNotificationCenter.default().addObserver(
            forName: Notification.Name(SpotifyNotification.playbackStateChanged),
            object: nil,
            queue: .main
        ) { [log] notification in
            // Flatten to Sendable strings before hopping to the main actor.
            let payload = ProbeFormat.flatten(notification.userInfo ?? [:])
            MainActor.assumeIsolated {
                var fields = ProbeFormat.dictionary(payload, expectedKeys: [
                    "Player State", "Name", "Artist", "Album", "Album Artist",
                    SpotifyNotification.trackID,
                    SpotifyNotification.duration,
                    SpotifyNotification.playbackPosition,
                    "Popularity", "Disc Number", "Track Number", "Play Count",
                    SpotifyNotification.hasArtwork,
                ])
                // Duration is in milliseconds, Playback Position in fractional seconds.
                fields.append(("⚠︎ units", "Duration = ms, Playback Position = s"))
                log.append(.spotify, "PlaybackStateChanged fired", fields: fields)
            }
        }
    }
