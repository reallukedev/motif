...
struct ActivityChart: View {
    ...
    private var xAxis: some AxisContent {
        switch unit {
        case .day where buckets.count <= 7:
        ...
        case .month where buckets.count <= 12:
            AxisMarks(values: .stride(by: .month)) { value in
                AxisValueLabel(format: .dateTime.month(.narrow), centered: true)
            }
        case .weekOfYear:
            AxisMarks(values: .stride(by: .weekOfYear, count: max(1, buckets.count / 4))) { value in
                AxisGridLine(stroke: StrokeStyle(lineWidth: 0.5))
                AxisValueLabel(format: .dateTime.day().month(.abbreviated))
            }
        default:
            AxisMarks(values: .stride(by: .month, count: max(1, buckets.count / 4))) { value in
                AxisGridLine(stroke: StrokeStyle(lineWidth: 0.5))
